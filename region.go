...
const (
	// probeConfirmTimeout is how long the region a token names gets to
	// answer on its own before the search widens to the rest of the map.
	// A miss here is not fatal: the widened search probes that region
	// again alongside the others.
	...
	// answers, for one of the regions the token already names to answer
	// too. A server reachable in several regions would otherwise have
	// its token rewritten to whichever relay happened to reply first.
	...
	// map can't turn one stale token into a storm.
	maxProbeRegions = 8
)

// DiscoverRegion returns a token equivalent to blob but naming the DERP
// region where the server is reachable now, for a server that has moved
// since blob was published. A server started with "genkey --region=auto"
// re-picks its region by latency at every startup, so a token handed out
// earlier can name a region it has since left; the client then just times
// out. The server's identity travels in the token and doesn't change, so
// DiscoverRegion keeps it and asks each candidate region whether the
// server is there, using the same meow handshake a real connection makes.
//
// If the server answers in the region blob already names, blob is returned
// unchanged.
//
...
func DiscoverRegion(ctx context.Context, blob ConnBlob, clientKey key.NodePrivate, opts ...any) (ConnBlob, error) {
	...
	// The search re-probes the regions the token names, so a confirm that
	// merely ran slow lands here rather than costing the connection.
	if got[0].named {
		return blob, nil
	}
	...
	// round trip through ConnBlob.
	ci.Region = nil
	ci.RegionID = got[0].reg.RegionID
	return ci.ConnBlob(), nil
}

// probeCandidate is a region to probe, and whether the token being
...
// not answer in the regions its token named: those regions first, then
...
//
// embedded says whether named came from the token rather than from dm. An
// embedded token's regions carry no usable ID — ConnBlob strips it and
// ParseConnBlob synthesizes 1..N in its place — so a dm region is
// recognized as one of them by relay hostname instead. A mismatch either
// way only costs one duplicate probe.
...
// (tailcat#7), and a region the token already names is probed alongside
// the rest, so several may answer. probe returns on the first answer,
// except that it waits probeSettle longer for a region the token names,
// whose answer means the token is still good and should not be rewritten.
// Callers must not read a single winner as proof the others are dead.
...
collect:
	for {
		...
		case c, ok := <-answers:
			...
			// Give a region the token names a moment to answer too,
			// rather than rewriting a token that still works.
