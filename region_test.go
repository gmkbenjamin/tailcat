...
func twoRegionDERPMap(t *testing.T) *tailcfg.DERPMap {
	...
	// uses IPv4, which stays loopback; HostName is what embedded-token
...
// staleBlob returns a token for priv's server naming regionID, whether
// or not the server is really there: the situation auto-region exists
// to repair.
func staleBlob(priv key.NodePrivate, regionID tailcfg.DERPRegionID) ConnBlob {
	ci := &ConnInfo{
...
func TestDiscoverRegion(t *testing.T) {
	...
	if got == old {
		t.Fatal("DiscoverRegion returned the stale blob unchanged")
	}
	ci, err := ParseConnBlob(got)
	if err != nil {
		t.Fatalf("parsing corrected blob: %v", err)
	}
	if ci.RegionID != 2 {
		t.Errorf("corrected RegionID = %v; want 2", ci.RegionID)
	}
	// Only the region may change; the identity is what makes the
	// corrected token refer to the same server.
	want, _ := ParseConnBlob(old)
...
func TestDiscoverRegionUnchanged(t *testing.T) {
	...
	if got != old {
		t.Errorf("DiscoverRegion changed a correct blob to %v", got)
	}
}
...
func TestDiscoverRegionAllowlist(t *testing.T) {
	...
	if ci, _ := ParseConnBlob(got); ci.RegionID != 2 {
...
func TestDiscoPublicMatchesMagicsock(t *testing.T) {
	...
	clientPriv := key.NewNode()
	c := &Client{Server: s.ConnBlob(), Key: clientPriv, Logf: mkLogger(t, "client")}
	t.Cleanup(func() { c.Close() })
...
func TestDiscoverRegionEmbedded(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()
	// A "--full-address" token for region 1: self-contained, and just as
	// stale as a short one once the server moves.
	old, err := staleBlob(srvPriv, 1).Resolve(ctx, dm)
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
	if ci, _ := ParseConnBlob(old); len(ci.Region) == 0 {
	...
	if len(ci.Region) != 0 {
		t.Errorf("corrected blob embeds %d regions; want the short form", len(ci.Region))
	}
}
...
func TestDiscoverRegionNotFound(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()
	_, err := DiscoverRegion(ctx, staleBlob(srvPriv, 1), key.NewNode(), dm, mkLogger(t, "discover"))
	if err == nil {
...
func TestDiscoverRegionMultiHomed(t *testing.T) {
	...
	if got != old {
		t.Errorf("DiscoverRegion changed a blob naming a live region to %v", got)
	}

	// Naming a region it isn't in: either of the others is a valid answer.
	got, err = DiscoverRegion(ctx, staleBlob(srvPriv, 3), key.NewNode(), dm, mkLogger(t, "discover"))
	if err != nil {
		t.Fatalf("DiscoverRegion from an unknown region: %v", err)
	}
	if ci, _ := ParseConnBlob(got); ci.RegionID != 1 && ci.RegionID != 2 {
...
func TestClientAutoRegion(t *testing.T) {
	...
	var gotBlob ConnBlob
	var gotRegion tailcfg.DERPRegionID
	var calls int
	c := &Client{
		Server:       staleBlob(srvPriv, 1),
		Logf:         mkLogger(t, "client"),
		AutoRegion:   true,
		DERPMapCache: staticDERPMapCacheForTest(t, dm),
		OnRegionDiscovered: func(b ConnBlob, r *tailcfg.DERPRegion) {
			calls++
			gotBlob, gotRegion = b, r.RegionID
		},
	}
	t.Cleanup(func() { c.Close() })
	...
	if got, ok := c.DiscoveredServer(); !ok || got != gotBlob {
		t.Errorf("DiscoveredServer = %v, %v; want %v, true", got, ok, gotBlob)
	}
...
func TestSearchCandidates(t *testing.T) {
	...
	if got, want := desc(searchCandidates(dm, named(dm.Regions[2]), false)), "2* 1 3"; got != want {
		t.Errorf("candidates for a token naming region 2 = %q; want %q", got, want)
	}
	...
	// An embedded token's region IDs are synthesized as 1..N, so the map
	...
	if got, want := desc(searchCandidates(dm, named(mk(1, "C.EXAMPLE.COM")), true)), "1* 1 2"; got != want {
		t.Errorf("candidates for an embedded token = %q; want %q", got, want)
	}
}
