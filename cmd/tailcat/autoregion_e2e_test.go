...
// relays, as regions 1 and 2. --auto-region needs that: a stale token
// names region 1 while the server is actually on 2.
...
func staleRegionBlob(t *testing.T, blob string, region tailcfg.DERPRegionID) string {
	t.Helper()
...
// a server bound to region 2, a token rewritten to name region 1,
// then ping without the flag (must fail) and with it (must find the
// server, print a pong on stdout, and put the corrected token on stderr).
func TestAutoRegionPing(t *testing.T) {
	...
	_, blob, _ := e.startServer("--key=" + keyFile)
	ci, err := tailcat.ParseConnBlob(tailcat.ConnBlob(blob))
	if err != nil {
		t.Fatal(err)
	}
	if ci.RegionID != 2 {
		t.Fatalf("server blob RegionID = %v; want 2", ci.RegionID)
	}
	stale := staleRegionBlob(t, blob, 1)
	...
	if !strings.Contains(stderr, "its current address is:") {
		t.Errorf("stderr = %q; want the corrected token", stderr)
	}
